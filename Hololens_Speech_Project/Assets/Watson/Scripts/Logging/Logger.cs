    ...
    public static class Log
    {
        ...
        public static void Debug(string subSystem, string messageFmt, params object[] args)
        {
			if(!LogSystem.InstalledDefaultReactors)
                System.Diagnostics.Debug.WriteLine("Call LogSystem.InstallDefaultReactors() prior to logging to see the output of Log.Debug(), Log.Status(), Log.Warning(), Log.Error() and Log.Critical().");            
            LogSystem.Instance.ProcessLog(new LogRecord(LogLevel.DEBUG, subSystem, messageFmt, args));
#if !NETFX_CORE
            System.Diagnostics.Debug.WriteLine("[{0}][{1}]: {2}", LogLevel.DEBUG, subSystem, string.Format(messageFmt, args));
#endif
        }
#else
...
#if !NETFX_CORE
            System.Diagnostics.Debug.WriteLine("[{0}][{1}]: {2}", LogLevel.STATUS, subSystem, string.Format(messageFmt, args));
#endif
        }
        /// <summary>
...
#if !NETFX_CORE
            System.Diagnostics.Debug.WriteLine("[{0}][{1}]: {2}", LogLevel.WARNING, subSystem, string.Format(messageFmt, args));
#endif
        }
        /// <summary>
...
#if !NETFX_CORE
            System.Diagnostics.Debug.WriteLine("[{0}][{1}]: {2}", LogLevel.ERROR, subSystem, string.Format(messageFmt, args));
#endif
        }
        /// <summary>
...
#if !NETFX_CORE
            System.Diagnostics.Debug.WriteLine("[{0}][{1}]: {2}", LogLevel.CRITICAL, subSystem, string.Format(messageFmt, args));
#endif
        }
    }
}
